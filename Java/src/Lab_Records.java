...
public class Lab_Records extends JFrame {
	...
	private void table_update(){
		...
		try {
			...
			String sql = "SELECT l.lr_name, l.lr_status, l.test_date, l.test_result FROM lab_report l where l.id = '"+no+"' ORDER BY test_date ASC;";
			...
			while(rs.next())
			{
				...
				for(c=1; c<=o; c++)
				{
					v2.add(rs.getString("lr_name"));
					v2.add(rs.getString("lr_status"));
					v2.add(rs.getString("test_date"));
					v2.add(rs.getString("test_result"));
				}
				Df.addRow(v2);
			}
			con.close();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Lab_Records frame = new Lab_Records();
	...
	public Lab_Records() {
		...
		JLabel lblMedicines_1 = new JLabel("Lab Records");
		...
		lblMedicines_1.setBounds(296, 0, 252, 88);
		...
		btnNewButton_1_1.setBounds(32, 24, 137, 40);
		...
		table.setBounds(53, 152, 816, 279);
