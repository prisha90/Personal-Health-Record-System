...
public class Medicine_Records extends JFrame {
	...
	private void table_update(){
		...
		try {
			...
			String sql = "SELECT m.med_name, m.strength, m.mtime FROM medicine m WHERE  (DATE(m.end_date) >= date(NOW()) AND TIME(m.mtime) >= TIME(NOW()) AND id = '"+no+"') ORDER BY end_date ASC, mtime ASC;";
			...
			while(rs.next())
			{
				...
				for(c=1; c<=o; c++)
				{
					...
					v2.add(rs.getFloat("strength"));
					v2.add(rs.getString("mtime"));
				}
				Df.addRow(v2);
			}
			con.close();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Medicine_Records frame = new Medicine_Records();
	...
	public Medicine_Records() {
		...
		JLabel lblMedicines = new JLabel("MEDICINES DUE TODAY");
		...
		table.setModel(new DefaultTableModel(
			...
			new String[] {
				"Name", "Strength", "Reminder Time"
			}
		));
		table.setFont(new Font("Times New Roman", Font.PLAIN, 15));
		table.setBounds(156, 143, 575, 232);
